...
if [ -d "$OPS_GLOBAL" ]; then
    mkdir -p $FLUX_REPO/global/automated/$GITHUB_REPOSITORY
    cp -r $OPS_GLOBAL $FLUX_REPO//global/automated/$GITHUB_REPOSITORY
    echo "[UA] Copying '$OPS_GLOBAL' files to flux repo."
fi

if [ -d "$OPS_US" ]; then
    mkdir -p $FLUX_REPO//us/automated/$GITHUB_REPOSITORY
    cp -r $OPS_US $FLUX_REPO//us/automated/$GITHUB_REPOSITORY
    echo "[UA] Copying '$OPS_US' files to flux repo."
fi

if [ -d "$OPS_CHINA" ]; then
    mkdir -p $FLUX_REPO//tokyo/automated/$GITHUB_REPOSITORY
    cp -r $OPS_CHINA $FLUX_REPO//tokyo/automated/$GITHUB_REPOSITORY
